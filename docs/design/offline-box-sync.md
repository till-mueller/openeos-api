...
**TSE/KassenSichV fiscal signing is affected — this is cloud-dependent, not a local concern.** The project's TSE strategy is cloud-only fiskaly (local hardware TSE was explicitly rejected — see `openeos-api` issue #8); fiskaly signs each transaction over its API, so a box with no uplink at all signs nothing live for the duration of the outage. That's not a bug: `TseService.recordTransaction` already treats this as the BMF Ausfall-Regelung (TSE-outage fallback) — the sale is never blocked, the payment is stored with `tseData.failed: true` and a reason, and the gap stays provable. For a Box, that means the *entire offline window* is one continuous, legally-flagged Ausfall period, not brief blips between otherwise-live signing. See `docs/design/tse-outage-reporting.md` for how those flagged payments get aggregated into an auditable Ausfallprotokoll and fed into the DSFinV-K export. **Caveat, not yet legally reviewed:** there's no known statutory cap on Ausfall duration, but a multi-day fully-offline event is untested territory — confirm with a tax advisor before relying on this for a real event, don't take this doc's word for it.
